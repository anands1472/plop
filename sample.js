...
rl.question("Enter the folder name: ", (folderName) => {
    ...
    // Define the apiUrl.js file path inside the config folder
    const apiUrlFilePath = path.join(configFolderPath, "apiUrl.js");

    // Content for apiUrl.js
    const apiUrlContent = `// apiUrl.js - Configuration for API URLs\n\nconst apiUrl = {\n  metaDataGatewayUri: "http://example.com/api/metaData",\n  kushaanGatewayUri: "http://example.com/api/kushaan",\n  commonServicesMicroServicesBaseUrL: "http://example.com/api/base",\n  kushaanMicroservicesUri: "/kushaan",\n};\n\nexport default apiUrl;`;

    // Create the apiUrl.js file
    fs.writeFileSync(apiUrlFilePath, apiUrlContent, "utf8");
    console.log(`✅ 'apiUrl.js' created inside 'config' folder.`);
